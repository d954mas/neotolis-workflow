...
1. Read this plan and the active task packet completely.
2. Read only the SPEC, runtime, and phase-contract sections relevant to that packet.
3. Never treat DESIGN-REFERENCES.md as requirements.
...
- Nearest ancestor containing a `.git` directory or worktree file is the consumer root. Read-only status uses native session `cwd` outside Git; `run start` rejects it.
...
src/core/{domain,protocol,errors,state,invariants}.ts
